package com.ccspart2.pokeswap_android.network.common

class Constants {
    companion object {
        const val POKEMON_BASE_URL = "https://api.pokemontcg.io/v2/"
        const val POKEMON_ENDPOINT = "https://api.pokemontcg.io/v2/cards/"

        const val EXCHANGE_RATE_BASE_URL = "https://v6.exchangerate-api.com/v6/"
        const val EXCHANGE_RATE_ENDPOINT = "https://v6.exchangerate-api.com/v6/976f4aab82216272247f7d90/latest/EUR/"
    }
}
